...
#### Note: 💡
##### To allow kubelet to work properly, we need to disable swap on both machines (Master and worker node).
...
### Important: 💡 Before you initialize cluster, you must config haproxy first. ❗️❗️❗️
...
## 8. Add FQDN to /etc/hosts
We need to add the FQDN address in `/etc/hosts`:
```bash
vim /etc/hosts
<HaProxyIPAddress> apisrv.aranetco.ir  
```
...
apt install haproxy
```
#### Then we should edit haproxy config.
```bash
